...
class Fotografo{
  ...
  function update_level($id, $level){
        $query =  $this->DB->exec("update fotografo set nivel='$level' where id=$id;");
  ...
  function listar_form_niveles(){
        try{
			...
			echo "<form action='niveles.php' method='post'>";
			echo "<tr>";
			echo "<td><a href='#'>Fotografo </a></td>"; 
			echo "<td> $rut </td>";
			echo "<td> $nombre </td>";
			echo "<td><input type='number' name ='level' min='1' max='4' value='$nivel' </td>";
			echo "<input type='hidden' name='id' value='$id'>";
			echo "<input type='hidden' name='nombre' value='$nombre'>";			
			echo "<td><button type='submit' name='change_level' class='btn btn-danger'>Modificar</button></td>";
			echo "</tr></form>";
